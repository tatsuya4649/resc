...
class CPUDetect(DetectBase):
    ...
    def __init__(
        ...
        if not isinstance(mode, str):
            raise CPUTypeError(
                (
                    "mode must be string type. "
                    f"({[x.value['name'] for x in CPUDetectMode]})"
                ))
        if len([x for x in CPUDetectMode
                if re.match(
                    rf'^{x.value["name"]}$',
                    f'{mode}',
            ...
            raise CPUValueError(
                ...
                {[x.value['name'] for x in CPUDetectMode]})"""
            )
        self._mode = [
            x for x in CPUDetectMode
            if re.match(
                rf'^{x.value["name"]}$',
                f'{mode}',
                flags=re.IGNORECASE
